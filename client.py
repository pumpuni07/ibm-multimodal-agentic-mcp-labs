"""
Connoisseur MCP Client
======================
IBM Skills Network Lab (Module 4, Lesson 2): "Build an MCP Client"

Connects to the Connoisseur MCP server over stdio, implements roots and
sampling callbacks, discovers tools/resources, and demos all three tools.

Requires ANTHROPIC_API_KEY in the environment (used only if the server
delegates a sampling request). Run with:
    python client.py
"""
